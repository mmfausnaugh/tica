...
The read/write permissions are required because GDrive uses Google accounts to manage access to public folders in the cloud.  Details about [DVC's privacy policy for Google APIs is available on the DVC webiste](https://dvc.org/doc/user-guide/privacy).  Note that you will only be able to read (pull) from the TICA calibration GDrive folder; you will not be able to write (push) to the TICA calibration folder.  DVC will prompt you to add the necessary Google account permissions the first time you try to pull TICA calibration models.  You can manage DVC's permissions at any time under the 'Security' tab in your Google account (under 'Third party apps with account access', see [this google help link](https://support.google.com/accounts/answer/3466521?hl=en) for more information).  
